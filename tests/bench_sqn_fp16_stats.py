"""FP16 FFT SQNR statistics — mean ± std over 100 trials per (N, signal) pair.

Measures forward FFT SQNR (dB) of FP16 cuFFT vs FP64 reference with optimal
complex scaling α (Bergach 2026 §IV-B).

Output: data/sqn-fp16-stats.csv + data/sqn-fp16-stats.md
"""
...
def main():
    if not torch.cuda.is_available():
        print("ERROR: CUDA required for cuFFT FP16 FFT.")
        sys.exit(1)
    ...
    csv_path = os.path.join("data", "sqn-fp16-stats.csv")
    md_path = os.path.join("data", "sqn-fp16-stats.md")
