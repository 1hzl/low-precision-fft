...
| GPU | SM | OS | Pytest 全量 | 消融 E4M3/config |
|-----|----|----|-------------|------------------|
| RTX 5090 | sm_120 | Linux | 4.6s | 27.6s |
| RTX 4090 | sm_89 | Linux | 5.4s | 37.5s |
| RTX 3090 | sm_86 | Linux | 5.6s | 37.5s |
| RTX 5060 Laptop | sm_120 | Win 11 | 6.1s | 134.6s |
| RTX 4060 Laptop | sm_89 | Win 11 | 6.9s | 27.8s |
| V100 | sm_70 | Linux | 8.4s | 53.9s |
| RTX 5070 Ti Laptop | sm_120 | Win 11 | 9.1s | 28.6s |

> Pytest 耗时受 CUDA 编译缓存影响（首次运行较慢）。消融实验为纯 Python BFP 计算，
> 耗时主要反映 CPU 单核性能（RTX 5060 的 134.6s 为 CPU 瓶颈，非 GPU 问题）。

### 4.2 FP16 vs FP32 cuFFT Throughput（RTX 5070 Ti, CUDA 13.3）
...
> **发现**: FP16 对小 N (<1K) 和大 N (>64K) 有 ~1.2-1.3× 加速。中等 N 受 Tensor Core
> 调度开销影响，加速不明显。最大相对误差 0.23% (N=1M)。

### 4.3 BFP FP8 vs FP16 Throughput（RTX 5070 Ti, batch=256）

| N | BFP (μs) | FP16 (μs) | BFP/FP16 | 说明 |
|---|----------|-----------|----------|------|
| 256 | 111.5 | 0.03 | 0.0003× | — |
| 1,024 | 137.1 | 0.03 | 0.0002× | — |
| 4,096 | 154.9 | 0.04 | 0.0002× | 2 bytes/elem vs 4 bytes |

> ⚠️ BFP FP8 为纯 Python 实现（非 CUDA 加速），不能与 cuFFT C++/CUDA 路径直接对比。
> BFP 适合内存受限场景（2 bytes/element），非吞吐优先场景。
> BFP CUDA kernel 加速方案在独立 Makefile 构建系统中。
...
|------|-------------|-------------|-------------|---------------|
| Pytest 耗时 | 8.4s | 5.6s | 5.4s | 4.6s |
| 消融 E4M3 耗时 | 53.9s | 37.5s | 37.5s | 27.6s |
| FP16 cuFFT 加速 | — | ~1.2×¹ | ~1.2×¹ | 1.3× (实测) |
